        ...
        >
          <div
            className={`w-6 h-6 rounded-full border-2 flex-shrink-0 flex items-center justify-center ${
              value === opt ? "border-teal-600 bg-teal-600" : "border-slate-300"
            }`}
          >
...
function SummaryRow({ label, value }: { label: string; value: string | string[] }) {
  const display = Array.isArray(value) ? value.join("、") || "なし" : value || "なし";
  return (
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6 bg-gradient-to-b from-teal-50 to-slate-50">
        <div className="text-center max-w-md">
          ...
          <button
            onClick={() => { setForm(initialData); setStep(0); setDone(false); }}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-teal-50 to-slate-50">
              ...
              <div key={i} className="flex flex-col items-center gap-1">
                <div
                  className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-colors ${
                    i < step ? "bg-teal-600 text-white" : i === step ? "bg-teal-600 text-white ring-4 ring-teal-200" : "bg-slate-200 text-slate-400"
                  }`}
                >
                  {i < step ? (
                ...
                <span className={`text-xs hidden sm:block ${i === step ? "text-teal-700 font-bold" : "text-slate-400"}`}>{label}</span>
              </div>
            ))}
          </div>
          <div className="h-1.5 bg-slate-200 rounded-full overflow-hidden">
            <div className="h-full bg-teal-500 rounded-full transition-all duration-500" style={{ width: `${(step / (STEPS.length - 1)) * 100}%` }} />
          </div>
        </div>
      </div>
      ...
      <main className="max-w-2xl mx-auto px-4 py-6 pb-32">
        {error && (
          <div className="mb-4 p-4 bg-rose-50 border border-rose-200 rounded-xl text-rose-700 text-sm font-medium">{error}</div>
        )}
        ...
        {step === 0 && (
          <div className="bg-white rounded-2xl shadow-sm p-5">
            ...
            <div className="space-y-5">
              <div><SectionLabel required>お名前</SectionLabel><TextInput value={form.name} onChange={(v) => set("name", v)} placeholder="例：山田 太郎" /></div>
              <div><SectionLabel>メールアドレス</SectionLabel><TextInput value={form.email} onChange={(v) => set("email", v)} placeholder="例：example@email.com" type="email" /></div>
              <div><SectionLabel required>性別</SectionLabel><RadioGroup options={["男性", "女性", "その他"]} value={form.gender} onChange={(v) => set("gender", v)} /></div>
              <div><SectionLabel required>生年月日</SectionLabel><TextInput value={form.birthDate} onChange={(v) => set("birthDate", v)} type="date" /></div>
              <div><SectionLabel required>住所</SectionLabel><TextArea value={form.address} onChange={(v) => set("address", v)} placeholder="例：埼玉県川越市○○町1-2-3" rows={2} /></div>
              <div><SectionLabel required>電話番号</SectionLabel><TextInput value={form.phone} onChange={(v) => set("phone", v)} placeholder="例：049-000-0000" type="tel" /></div>
            </div>
          </div>
        )}
        ...
        {step === 1 && (
          <div className="bg-white rounded-2xl shadow-sm p-5">
            ...
            <div className="space-y-6">
              <div><SectionLabel required>左右どちらの目の症状ですか？</SectionLabel><RadioGroup options={["右眼", "左眼", "両眼"]} value={form.affectedEye} onChange={(v) => set("affectedEye", v)} /></div>
              <div>
                <SectionLabel required>どのような症状ですか？（複数選択可）</SectionLabel>
                <CheckboxGroup options={SYMPTOMS} values={form.symptoms} onChange={(v) => set("symptoms", v)} />
                <div className="mt-3"><TextInput value={form.symptomsOther} onChange={(v) => set("symptomsOther", v)} placeholder="その他の症状があれば記入" /></div>
              </div>
              <div><SectionLabel required>いつから症状がありますか？</SectionLabel><TextInput value={form.symptomOnset} onChange={(v) => set("symptomOnset", v)} placeholder="例：1週間前から、昨日から" /></div>
            </div>
          </div>
        )}
          ...
          <div className="bg-white rounded-2xl shadow-sm p-5">
            ...
            <div className="space-y-6">
              <div>
                ...
                <CheckboxGroup options={PAST_EYE_DISEASES} values={form.pastEyeDiseases} onChange={(v) => set("pastEyeDiseases", v)} />
                <div className="mt-3"><TextInput value={form.pastEyeDiseasesOther} onChange={(v) => set("pastEyeDiseasesOther", v)} placeholder="その他（具体的に）" /></div>
              </div>
              <div><SectionLabel>現在通院中の眼科はありますか？</SectionLabel><TextInput value={form.currentEyeClinic} onChange={(v) => set("currentEyeClinic", v)} placeholder="眼科名を入力（なければ空白）" /></div>
              <div>
                <SectionLabel>現在・過去にかかっている全身の病気（複数選択可）</SectionLabel>
                <CheckboxGroup options={GENERAL_DISEASES} values={form.generalDiseases} onChange={(v) => set("generalDiseases", v)} />
                <div className="mt-3"><TextInput value={form.generalDiseasesOther} onChange={(v) => set("generalDiseasesOther", v)} placeholder="その他の病気（具体的に）" /></div>
              </div>
              <div><SectionLabel>現在飲んでいる薬・サプリ・目薬</SectionLabel><TextInput value={form.medications} onChange={(v) => set("medications", v)} placeholder="なければ空白" /></div>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="bg-white rounded-2xl shadow-sm p-5">
            ...
            <div className="space-y-6">
              <div><SectionLabel>アレルギー（食べ物・薬・注射）</SectionLabel><TextInput value={form.allergies} onChange={(v) => set("allergies", v)} placeholder="なければ空白" /></div>
              <div><SectionLabel>お薬手帳に貼る紙の発行を希望しますか？</SectionLabel><RadioGroup options={["希望する", "希望しない"]} value={form.medicationBooklet} onChange={(v) => set("medicationBooklet", v)} /></div>
              <div><SectionLabel>普段、メガネ・コンタクトを使用していますか？（複数選択可）</SectionLabel><CheckboxGroup options={VISION_CORRECTION} values={form.visionCorrection} onChange={(v) => set("visionCorrection", v)} columns={1} /></div>
              <div><SectionLabel>妊娠中または授乳中ですか？（該当するものを選択）</SectionLabel><CheckboxGroup options={PREGNANCY} values={form.pregnancy} onChange={(v) => set("pregnancy", v)} columns={1} /></div>
            </div>
          </div>
        )}
        ...
        {step === 4 && (
          <div className="space-y-4">
            <div className="bg-teal-50 border border-teal-200 rounded-2xl p-4">
              <p className="text-teal-800 text-sm font-medium">以下の内容をご確認の上、「送信する」ボタンを押してください。修正する場合は「戻る」ボタンをご利用ください。</p>
            </div>
            <div ref={summaryRef} className="bg-white rounded-2xl shadow-sm p-5">
              <h3 className="text-base font-bold text-teal-700 mb-4 pb-3 border-b-2 border-teal-100">川越あさひ眼科　問診票　確認</h3>
              ...
              <SummaryRow label="症状" value={[...form.symptoms, ...(form.symptomsOther ? [form.symptomsOther] : [])]} />
              ...
              <SummaryRow label="過去の目の病気" value={[...form.pastEyeDiseases, ...(form.pastEyeDiseasesOther ? [form.pastEyeDiseasesOther] : [])]} />
              <SummaryRow label="通院中の眼科" value={form.currentEyeClinic} />
              <SummaryRow label="全身の病気" value={[...form.generalDiseases, ...(form.generalDiseasesOther ? [form.generalDiseasesOther] : [])]} />
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 p-4 shadow-lg z-20">
          <div className="max-w-2xl mx-auto flex gap-3">
            {step > 0 && (
              <button onClick={handleBack} className="flex-1 py-4 rounded-xl border-2 border-slate-300 text-slate-700 font-bold text-base active:bg-slate-50">
                ← 戻る
              </button>
            )}
            {step < STEPS.length - 1 ? (
              <button onClick={handleNext} className="flex-[2] py-4 rounded-xl bg-teal-600 text-white font-bold text-base shadow-md active:bg-teal-700">
                次へ →
              </button>
            ) : (
              <button onClick={handleSubmit} disabled={sending} className="flex-[2] py-4 rounded-xl bg-teal-600 text-white font-bold text-base shadow-md disabled:opacity-60 active:bg-teal-700">
                {sending ? "送信中..." : "送信する"}
              </button>
            )}
          </div>
